...
def add_contact(request):
    ...
    if form.is_valid() and request.method == "POST":
        ...
        contact.save()
        messages.add_message(request=request, level=messages.SUCCESS, message="Contact added successfully!")
        return redirect("get_contacts")
    return render(request=request, template_name="add_contact.html", context=dict(form=form))


@login_reqireted(login_url="/sign_in/")
def get_contacts(request):
    contacts = Contact.objects.filter(user=request.curent_user).all()
